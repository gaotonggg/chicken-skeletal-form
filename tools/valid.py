...
def main():
    ...
    out = cv2.VideoWriter('valid4.mp4',fourcc, 3.0, (1080,1080),True)
    for i in range(100000)[2:13956]: #13956 5771 7684 9504 10285
        image = cv2.imread('/media/gg/2T/HRNet/HigherHRNet-Human-Pose-Estimation/data/coco/images/train2017_x/'+str(i)+".png")
        ...
        if True:#i % cfg.PRINT_FREQ == 0:
            # prefix = '{}_{}'.format(os.path.join(final_output_dir, 'result_valid'), i)
            prefix = '{}_{}'.format('result_valid', i)
            # logger.info('=> write {}'.format(prefix))
            img = save_valid_image(image, final_results, '{}.jpg'.format(prefix), dataset=test_dataset.name)
            # out.write(img)
